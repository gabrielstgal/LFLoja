...
const ProductCard = ({ product }) => {
  ...
  const lowStock = product.quantidadeEstoque > 0 && product.quantidadeEstoque <= 5;
  const outOfStock = product.quantidadeEstoque <= 0;
  const hasPromo = product.precoPromocional && product.precoPromocional < product.preco;
  const discountPercent = hasPromo ? Math.round((1 - product.precoPromocional / product.preco) * 100) : 0;
  ...
  return (
    <div className="product-card" onClick={handleCardClick}>
      <div className="product-image-container">
        ...
        {hasPromo && <span className="product-badge product-badge-promo">-{discountPercent}%</span>}
        {lowStock && !hasPromo && <span className="product-badge product-badge-low">Últimas unidades</span>}
